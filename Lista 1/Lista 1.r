# Método de Tauchen
    ...
    theta_min <- -theta_max
    # dividindo [theta_min, theta_max] em n pontos
    ...
    # tamanho dos saltos entre os pontos do grid
    delta <- (max(grid) - min(grid)) / (length(grid) - 1)


    # vetor de transição para os valores extremos do grid dado cada estado anterior; pnorm(x) retorna a cdf da normal no valor x
