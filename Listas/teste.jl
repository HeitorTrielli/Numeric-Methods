...
value_function_egm = function(;c0::Array{Float64,2} = c0, v0 = zeros(500,7), k::Array{Float64,1} = grid_k, tol::Float64 = 1e-4,
    beta::Float64 = 0.987, mu::Float64 = 2.0, alpha::Float64 = 1 / 3, delta::Float64 = 0.012, rho::Float64 = 0.95, sigma::Float64 = 0.007, inert::Float64 = 0.0,
    kss::Float64 = k_ss, k_len::Int64 = 500, z_len::Int64 = 7, inert = 0.5)
...
####################################################################################################################################################################################
plotada = function(x; label = true, tit = " ", extra = " ")
    if label == true
        labels = ["State 1" "State 2" "State 3" "State 4" "State 5" "State 6" "State 7"]
        p1 = plot(x[2], x[1], label = labels, legend = :topleft, title = tit)
        p2 = plot(x[2], label = labels, legend = :topleft, title = tit)
        p3 = plot(x[2], x[3], label = labels, legend = :topleft, title = tit)
    else
        labels = ["" "" "" "" "" "" ""]        
        p1 = plot(x[2], x[1], label = labels, legend = :topleft, title = tit)
        p2 = plot(x[2], label = labels, legend = :topleft, title = tit)
...
accelerator_ = @time value_function_mg(100, 500, 500)
...
println("Monotone")
monotone = @benchmark value_function_monotone() seconds = 300


println("Concavity")
concave = @benchmark value_function_concave() seconds = 300


println("Monotone + Concavity")
vf = @benchmark value_function() seconds = 300


println("Accelerator")
accelerator = @benchmark value_function_accelerator() seconds = 300
...
println("EGM")
egm = @benchmark value_function_egm(c0 = accelerator_[3]) seconds = 300

accelerador_[3]
teste = value_function_egm(c0 = 15*c0)
teste2 = value_function_egm(c0 = accelerator[3])
teste3 = value_function_egm()
plot(brute_force[2][1:100,:], title = "Função Política Brute Force zoom",  legend = :topleft, label = labels)

plot(EEE2(teste[3], teste[2], teste[5]),  title = "Grid Endógeno, chute baixo",  legend = :bottomleft, label = labels)
...
egm = value_function_egm()
accelerator_brute = value_function_accelerator_brute()


plotbrute = plotada(brute_force, tit = "Brute Force")
plotmon = plotada(monotone, label = false, tit = "Monotone")
plotconc = plotada(concave, label = false, tit = "Concavity")
plotvf = plotada(vf, label = false, tit =  "Concavity + Monotone")
plotaccel = plotada(accelerator_, tit = "Accelerator")
plotaccelb = plotada(accelerator_brute, tit = "Accelerator Brute")
plotmg = plotada(multigrid, tit = "Multigrid")
plotegm = plotada(egm, tit = "Gird Endógeno")
plotegm[3]
labels = ["State 1" "State 2" "State 3" "State 4" "State 5" "State 6" "State 7"]

plot(EEE(brute_force), title = "Brute Force", label = labels)
plot(EEE(monotone), title = "Monotone")
plot(EEE(concave), title = "Concave")
plot(EEE(vf), title = "Monotone + Concave", label = labels)
plot(EEE(accelerator_), title = "Accelerator", label = labels)

plot(multigrid[2], multigrid[1], title = "Multigrid", label = labels, legend = :topleft)
plot(multigrid[2], title = "Multigrid", label = labels, legend = :topleft)
plot(multigrid[2], multigrid[3], title = "Multigrid", label = labels, legend = :topleft)
